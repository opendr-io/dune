...
def normalize_filter_pair(event_source, event_name):
    ...
    if not event_source:
        return None
    return event_source, event_name or "*"
...
def record_matches_filter(record, filter_pairs):
    ...
    for source_pattern, name_pattern in filter_pairs:
        if fnmatchcase(event_source, source_pattern) and fnmatchcase(event_name, name_pattern):
            return True
    return False
...
def run_once(args, worker_count):
    ...
    if filter_pairs:
        print(f"Dropping records matching {len(filter_pairs):,} eventSource/eventName filter rule(s)")
